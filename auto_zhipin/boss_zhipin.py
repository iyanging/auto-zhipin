...
async def default_job_filter(job: Job) -> bool:
...
class BossZhipin(AbstractAsyncContextManager["BossZhipin"]):
    ...
    async def query_jobs(
        ...
        *,
        ...
        encrypt_job_id_to_job_summary = {}
        security_id_to_job_detail = {}
        ...
        async def on_request_finished(req: PlaywrightRequest) -> None:
            if req.url.startswith(self._get_job_list_url_prefix()):
                ...
                security_id_to_job_detail[job_detail.security_id] = job_detail
        ...
        async with await ctx.new_page() as page:
            ...
            await job_list_con.hover()  # 指针移动到职位列表中央

            queried_count = 0
            prev_height = 0

            while queried_count < count:
                bbox = await job_list_con.bounding_box()
                if bbox is None:
                    raise ProgrammingError("Cannot locate bbox")

                await page.mouse.wheel(0, bbox["height"] - prev_height)
                ...
                if bbox["height"] > prev_height:
                    prev_height = bbox["height"]
                else:
                    break

            jobs = await job_list_con.locator(".job-card-box").all()
            for job in jobs:
                if queried_count >= count:
                    break
